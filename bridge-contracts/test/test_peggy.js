const CashPool = artifacts.require('CashPool');
...
contract('CashPool', function (accounts) {
  ...
  describe('CashPool smart contract deployment', function(){

    beforeEach(async function() {
      this.cashPool = await CashPool.new();
    });

    it('should deploy the cashPool contract with the correct parameters', async function () {
      this.cashPool.should.exist;

      const cashPoolRelayer = (await this.cashPool.relayer());
      cashPoolRelayer.should.be.equal(relayer);
    });
  ...
  describe('Locking funds', function(){
    ...
    it('should not allow a user to send ethereum directly to the contract', async function () {
      await this.cashPool.send(this.weiAmount, { from: userOne}).should.be.rejectedWith(EVMRevert);
    });
    ...
    it('should not allow users to lock funds if the contract is paused', async function () {
      ...
      await this.cashPool.pauseLocking({ from:relayer }).should.be.fulfilled;
      
      const depositStatus = await this.cashPool.active();
      depositStatus.should.be.equal(false);

      //User attempt to lock ethereum/erc20
      await this.token.approve(this.cashPool.address, 100, { from: userOne }).should.be.fulfilled;
      await this.cashPool.lock(this.recipient, this.token.address, 100, { from: userOne, gas: this.gasForLock }).should.be.rejectedWith(EVMRevert);
    });
    ...
    it('should allow users to lock erc20 tokens if it meets validation requirements', async function () {
      ...
      await this.token.approve(this.cashPool.address, 100, { from: userOne }).should.be.fulfilled;
      await this.cashPool.lock(this.recipient, this.token.address, 100, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;

      //Get the contract and user token balance after the rescue
      const cashPoolBalance = Number(await this.token.balanceOf(this.cashPool.address));
      const userBalance = Number(await this.token.balanceOf(userOne));

      //Confirm that the tokens have been locked
      cashPoolBalance.should.be.bignumber.equal(100);
    ...
    it('should allow users to lock ethereum if it meets validation requirements', async function () {
      ...
      await this.cashPool.lock(this.recipient, this.ethereumToken, this.weiAmount, { from: userOne, value: this.weiAmount, gas: this.gasForLock }).should.be.fulfilled;

      const contractBalanceWei = await web3.eth.getBalance(this.cashPool.address);
    ...
    it('should emit an event upon lock containing the new ecrow\'s information', async function () {
      ...
      const expectedId = await this.cashPool.lock.call(this.recipient, this.token.address, 100, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;
      const {logs} = await this.cashPool.lock(this.recipient, this.token.address, 100, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;
  ...
  describe('Access to information', function(){
    ...
    beforeEach(async function() {
      ...
      await this.token.approve(this.cashPool.address, 100, { from: userOne }).should.be.fulfilled;
      this.itemId = await this.cashPool.lock.call(this.recipient, this.token.address, 100, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;
      await this.cashPool.lock(this.recipient, this.token.address, 100, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;
    });

    it('should allow for public viewing of a locked item\'s information', async function () {
      //Get the item struct's information
      const itemInfo = await this.cashPool.viewItem(this.itemId, { from: relayer }).should.be.fulfilled;
  ...
  describe('Unlocking of itemized ethereum', function(){
    ...
    it('should allow the relayer to unlock itemized ethereum', async function () {
      const id = await this.cashPool.lock.call(this.recipient, this.ethereumToken, this.weiAmount, { from: userOne, value: this.weiAmount, gas: this.gasForLock }).should.be.fulfilled;
      await this.cashPool.lock(this.recipient, this.ethereumToken, this.weiAmount, { from: userOne, value: this.weiAmount, gas: this.gasForLock }).should.be.fulfilled;
      await this.cashPool.unlock(id, { from: relayer, gas: this.gasForLock }).should.be.fulfilled;
    });

    it('should allow the relayer to unlock itemized erc20 tokens', async function () {
      await this.cashPool.unlock(this.itemId, { from: relayer, gas: this.gasForLock }).should.be.fulfilled;
    });
    ...
    it('should correctly transfer funds to intended recipient upon unlock', async function () {
      ...
      const beforecashPoolBalance = Number(await this.token.balanceOf(this.cashPool.address));
      const beforeUserBalance = Number(await this.token.balanceOf(userOne));

      beforecashPoolBalance.should.be.bignumber.equal(100);
      beforeUserBalance.should.be.bignumber.equal(0);

      await this.cashPool.unlock(this.itemId, { from: relayer, gas: this.gasForLock });
      
      //Confirm that the tokens have been unlocked and transfered
      const aftercashPoolBalance = Number(await this.token.balanceOf(this.cashPool.address));
      const afterUserBalance = Number(await this.token.balanceOf(userOne));

      aftercashPoolBalance.should.be.bignumber.equal(0);
    ...
    it('should emit an event upon unlock containing the ecrow\'s recipient, token, amount, and nonce', async function () {
      ...
      const {logs} = await this.cashPool.unlock(this.itemId, { from: relayer, gas: this.gasForLock });
    ...
    it('should update item lock statusit has been unlocked', async function () {
      const startingLockStatus = await this.cashPool.getStatus(this.itemId);
      startingLockStatus.should.be.equal(true);
      
      await this.cashPool.unlock(this.itemId, { from: relayer, gas: this.gasForLock });

      const endingLockStatus = await this.cashPool.getStatus(this.itemId);
  ...
  describe('Withdrawal of items by sender', function(){
    ...
    it('should not allow non-senders to withdraw other\'s items', async function () {
      await this.cashPool.withdraw(this.itemId, { from: userThree, gas: this.gasForLock }).should.be.rejectedWith(EVMRevert);
    });

    it('should allow senders to withdraw their own items', async function () {
      await this.cashPool.withdraw(this.itemId, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;
    });
    ...
    it('should return erc20 to user upon withdrawal of itemized funds', async function () {
      ...
      await this.cashPool.withdraw(this.itemId, { from: userOne, gas: this.gasForWithdraw }).should.be.fulfilled;
    ...
    it('should return ethereum to user upon withdrawal of itemized funds', async function () {
      const id = await this.cashPool.lock.call(this.recipient, this.ethereumToken, this.weiAmount, { from: userTwo, value: this.weiAmount, gas: this.gasForLock }).should.be.fulfilled;
      await this.cashPool.lock(this.recipient, this.ethereumToken, this.weiAmount, { from: userTwo, value: this.weiAmount, gas: this.gasForLock }).should.be.fulfilled;
      ...
      //Get prior balances of user and cashPool contract
      const beforeUserBalance = Number(await web3.eth.getBalance(userTwo));
      const beforeContractBalance = Number(await web3.eth.getBalance(this.cashPool.address));

      //Send withdrawal transaction and save gas expenditure
      const txHash = await this.cashPool.withdraw(id, { from: userTwo, gas: this.gasForWithdraw}).should.be.fulfilled;
      const gasCost = this.gasPrice * txHash.receipt.gasUsed;

      //Get balances after withdrawal
      const afterUserBalance = Number(await web3.eth.getBalance(userTwo));
      const afterContractBalance = Number(await web3.eth.getBalance(this.cashPool.address));
    ...
    it('should emit an event upon user withdrawal containing the item\'s id', async function () {      
      ...
      const {logs} = await this.cashPool.withdraw(this.itemId, { from: userOne, gas: this.gasForLock }).should.be.fulfilled;
