...
### System Process:
1. Users lock Ethereum or ERC20 tokens on the CashPool contract, resulting in the emission of an event containing the created item's original sender's Ethereum address, the intended recipient's Cosmos address, the type of token, the amount locked, and the item's unique nonce.
...
The Relayer service and Oracle module are under development here: https://github.com/xbridge-finance/poc-cosmos.
...
Note: This project currently uses solc@0.8.19, make sure that this version of the Solidity compiler is being used to compile the contracts and does not conflict with other versions that may be installed on your machine.

## Testing
Run commands from the appropriate directory: `$ cd bridge-contracts`  
...
Thanks to @adrianbrink, @mossid, and @sunnya97 for contributions to the original CashPool repository.
